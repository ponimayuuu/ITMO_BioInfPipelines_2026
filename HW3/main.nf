...
workflow variant_calling_wf {
    ...
    main:
        bam_ch
            .join(bai_ch)
            .join(reference_ch)
            .map { label, bam, bai, fasta ->
                tuple( [id: label], bam, bai, fasta )
            }
            .set { combined_ch }

        BCFTOOLS_MPILEUP(
            combined_ch.map { meta, bam, bai, fasta -> tuple(meta, bam, bai) },
            combined_ch.map { meta, bam, bai, fasta -> tuple(meta, fasta) },
            params.save_mpileup
        )
    emit:
